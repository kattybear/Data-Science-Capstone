...
app.layout = html.Div(children=[html.H1('SpaceShit Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider', min = 0, max = 10000, step = 1000,
                                                value = [min_payload, max_payload], marks={ 2500: {'label': '2500 (Kg)'},
                                                5000: {'label': '5000 (Kg)'}, 7500: {'label': '7500 (Kg)'}}),
...
def select(input_pg):
    if input_pg == 'All Sites':
        new_df1 = spacex_df.groupby(['Launch Site'])["class"].sum().to_frame()
        new_df1 = new_df1.reset_index()
        fig1 = px.pie(new_df1, values='class', names='Launch Site', title='Total Success Launches by Site')
    else:
        new_df1 = spacex_df[spacex_df["Launch Site"] == input_pg]["class"].value_counts().to_frame()
        new_df1["name"] = ["Failure", "Success"]
        fig1 = px.pie(new_df1, values='class', names='name', title='Total Success Launches for ' + input_pg)
    return fig1
